...
class ArgMaxClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        # ====== YOUR CODE: ======
        output = y_proba.argmax(dim=1)
        assert len(output.shape) == 1
        assert output.shape[0] == y_proba.shape[0] 
        assert output.dtype in (torch.int32, torch.int64)
        return output
        # ========================
...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        #  Output should be a (N,) integer tensor.
        # ====== YOUR CODE: ======
        z = torch.where(y_proba[:, self.positive_class] > self.threshold, torch.tensor([1]), torch.tensor([0]))
        return z.int()
        # ========================
